...
KEYTAB_NAME='yarn.service.keytab'

for LINE in `awk '/^[^#]/ { print }' datanode | cat`  
do   
        echo "ipa-getkeytab -s ${ipa_server} -p yarn/${LINE}@${RLEAM} -k ${KEYTAB_NAME}"
        echo "chown yarn:hadoop ${KEYTAB_NAME};chmod 400 ${KEYTAB_NAME}"
